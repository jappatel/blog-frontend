  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:5000/api/blogs')
      .then(response => {
        ...
        const foodBlog = response.data.filter(blog => blog.subject === 'Food' || blog.subject === 'food');
        // Assuming you want only the first 3 technology blogs
        setBlogs(foodBlog.slice(0, 4));
      })
      .catch(error => {
...
const handleBtnClick = async (selectedBlog) =>{
  ...
  return (
   ...
   {Blogs.map(blog => (
        <div className='mb-10  w-1/2 space-x-4  ' key={blog._id} >
          <div >
               {extractFirstImgTag(blog.content).map((imgTag, index) => (
               <div className=' rounded-xl overflow-hidden   ' key={index} dangerouslySetInnerHTML={{ __html: imgTag }} />
               ))}
          </div>
